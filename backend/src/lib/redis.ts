...
/** Graceful shutdown: ulangan bo'lsa redis klientini yopadi. */
export async function closeRedis(): Promise<void> {
  const client = globalRedis.__pfosRedisClient;
  globalRedis.__pfosRedisClient = undefined;
  globalRedis.__pfosRedisConnecting = undefined;
  try {
    if (client?.isOpen) await client.disconnect();
  } catch {
    /* yopishda xato — jimgina e'tiborsiz */
  }
}
