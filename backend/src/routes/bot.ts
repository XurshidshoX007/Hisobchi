...
export async function POST(request: Request) {
  ...
  if (isProduction()) {
    return withSecurityHeaders(Response.json({ error: "Not found", code: "not_found" }, { status: 404 }), sec.requestId);
  }
  try {
    ...
    try {
      ...
      if (error instanceof PayloadTooLargeError) {
        return withSecurityHeaders(
          Response.json({ text: "Request juda katta.", keyboard: [], code: "payload_too_large" }, { status: 413 }),
          sec.requestId,
        );
      }
      if (error instanceof SyntaxError || error instanceof TypeError) {
        return withSecurityHeaders(
          Response.json({ text: "Request formati noto'g'ri.", keyboard: [], code: "invalid_json" }, { status: 400 }),
          sec.requestId,
        );
      }
      throw error;
    ...
    if (typeof body.message === "string" && body.message.length > 4_096) {
      return withSecurityHeaders(
        Response.json({ text: "Xabar juda uzun.", keyboard: [], code: "invalid_input" }, { status: 400 }),
        sec.requestId,
      );
    }
    ...
    return withSecurityHeaders(Response.json(reply), sec.requestId);
  } catch {
    return withSecurityHeaders(
      Response.json(
