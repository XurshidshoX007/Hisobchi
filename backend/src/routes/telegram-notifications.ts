...
import { todayISO } from "@hisobchi/shared/lib/money";
...
export async function POST(request: Request) {
  ...
  if (!redis) {
    ...
    return Response.json({ ok: false, code: "redis_unavailable" }, { status: 503 });
  }
  ...
  try {
    ...
    return Response.json({ ok: failed === 0, sent, skipped, failed, requestId: sec.requestId }, { status: failed ? 207 : 200 });
  } catch {
    securityLog("error", "notification_dispatch_error", { requestId: sec.requestId, code: "internal" });
    return Response.json({ ok: false, code: "internal", requestId: sec.requestId }, { status: 500 });
  }
}
