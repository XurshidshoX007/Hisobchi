...
import { ACK, batchSummary, BUTTON, draftSummary, MINI_APP_INTRO, SCHEDULE } from "@hisobchi/shared/lib/bot-copy";
...
import { shortDate } from "@hisobchi/shared/lib/money";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!claimed[0]) return Response.json({ ok: true, idempotent: true });
    ...
    if (text.length > 4_096) return Response.json({ ok: true });
    ...
    return Response.json({ ok: failure.code === "invalid_json" }, { status: failure.status });
  }
}
