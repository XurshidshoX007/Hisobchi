...
import { totalBalanceInCurrency } from "@hisobchi/shared/lib/finance";
...
test("an ambiguous mutation exception keeps its idempotency claim", () => {
  const route = readFileSync(new URL("../src/routes/mutate.ts", import.meta.url), "utf8");
...
test("the Mini App reuses an idempotency key after an ambiguous response", () => {
  const provider = readFileSync(new URL("../../frontend/src/components/providers.tsx", import.meta.url), "utf8");
...
test("schedule idempotency never converts a database exception into duplicate success", () => {
  const webhook = readFileSync(new URL("../src/routes/telegram-webhook.ts", import.meta.url), "utf8");
...
test("ledger currency cannot be relabelled or mixed without an FX model", () => {
  ...
  const settings = readFileSync(new URL("../../frontend/src/pages/settings.tsx", import.meta.url), "utf8");
