...
import { buildAnalytics, computeLedgerBalances, type AccountView } from "@hisobchi/shared/lib/finance";
...
test("sync and user-isolation guards remain in the shared state/mutation paths", () => {
  ...
  const bot = readFileSync(new URL("../../backend/src/lib/bot.ts", import.meta.url), "utf8");
  const mutations = readFileSync(new URL("../../backend/src/lib/mutations.ts", import.meta.url), "utf8");
