  ...
  try {
    ...
    if (!user) {
      ...
      return withSecurityHeaders(
        NextResponse.json({ error: "Autentifikatsiya talab qilinadi", code: "unauthorized" }, { status: 401 }),
        sec.requestId,
      );
    }
    ...
    return withSecurityHeaders(NextResponse.json(state), sec.requestId);
  } catch (error) {
    securityLog("error", "state_error", { requestId: sec.requestId, ipKey: sec.ipKey, code: "internal" });
    return withSecurityHeaders(
      NextResponse.json({ error: "Ma'lumotlarni yuklashda xatolik", code: "internal" }, { status: 500 }),
      sec.requestId,
    );
  }
}
